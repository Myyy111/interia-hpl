...
async function check() {
    const { data } = await supabase.from('website_data').select('*');
    if (data) console.log('All Website Data rows count:', data.length);
    if(data) data.forEach(r => console.log('row id:', r.id));
}
check();
